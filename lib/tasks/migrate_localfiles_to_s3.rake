...
def migrate(from, to, organization)
  ...
  other_assets = []
  organization_ck_assets.flatten.each do |ock|
    blobs = []
    ckeditor_assets.each{|a| blobs << a if a.filename.to_s != ock}
    other_assets << blobs
  end
  other_assets.flatten.each do |ock_blob|
    begin
      p ock_blob.blob
      print '.'
      file = Tempfile.new("file#{Time.now}")
      file.binmode
      file << ock_blob.blob.download
      file.rewind
      checksum = ock_blob.blob.checksum
      to_service.upload(ock_blob.blob.key, file, checksum: checksum)
    rescue Errno::ENOENT
      puts "Rescued by Errno::ENOENT statement. ID: #{ock_blob.blob.id} / Key: #{ock_blob.blob.key}"
      next
    rescue ActiveStorage::FileNotFoundError
      puts "Rescued by FileNotFoundError. ID: #{ock_blob.blob.id} / Key: #{ock_blob.blob.key}"
      next
    end
  end
end
